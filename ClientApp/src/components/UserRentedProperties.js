...
// Same as UserProperties, but "items.filter(item => item.publisherUserName === userName)" instead of items.filter(item => item.bookerUserName === userName)
function UserRentedProperties() {
    ...
    const renderUserRentedProperties = (items, userName) => (
        <>
            <table className='table' style={{ borderCollapse: 'separate', borderSpacing: '0px 2rem' }} aria-labelledby="tabelLabel">
                <thead>
                    <tr>
                        ...
                        <th>To Date</th>
                        <th>Total Days</th>
                        <th>Booker UserName</th>
                        <th>Total Price</th>
                    </tr>
                </thead>
                <tbody>
                    { /* Filtering based on key values, item.publisherUserName and userName */}
                    { /* Source: https://stackabuse.com/how-to-filter-an-object-by-key-in-javascript/ */}
                    {items.filter(item => item.publisherUserName === userName)
    ...
    return (
        <div className="box">
            <br></br>
            <h1 id="tabelLabel">My Rented Out Properties</h1>
            <br></br>
            {loading ? <p><em>Loading...</em></p> : renderUserRentedProperties(items, userName)}
            <br></br>
        </div>
